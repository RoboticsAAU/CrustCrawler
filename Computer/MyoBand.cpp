...
//Function that returns a vector with the 8 EMG samples.
std::vector<int8_t> MyoBand::getEMGdata() {
    //If the Myo isn't paired, on the arm, or unlocked, we will return a vector with zeros. 
    //Otherwise the actual EMG values are returned  
    if ( !isPaired || !onArm || !isUnlocked ) {
...
void MyoBand::print()
{
    //We get the current pose and print it
